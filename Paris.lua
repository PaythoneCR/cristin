...
ControllerAll = false
local ListSudos ={Sudo_Id,1001132193,1833610203,88888888888888,99999999999999,77777777777777}  
...
Status = 'DEV • CRISTIN'
elseif UserId == 88888888888888 then  
Status = 'المبرمج كريستينن'
...
msg.The_Controller = 1
elseif tonumber(msg.sender.user_id) == 88888888888888 then
msg.Name_Controller = 'المبرمج كريستينن'
msg.The_Controller = 1
...
local N = (Redis:get(paris.."paris:Name:Bot") or "كريستين")
...
--download('https://raw.githubusercontent.com/PaythoneCR/cristin/blob/main/paris.lua')
...
NamesBot = (Redis:get(paris.."paris:Name:Bot") or "كريستين")
...
if Controller(msg_chat_id,UserId_Info.id) == 'المبرمج كريستينن' then
return LuaTele.sendText(msg_chat_id,msg_id,"\n* ❍︙ عذرا لا تستطيع حظر عام〘 "..Controller(msg_chat_id,UserId_Info.id).." 〙*","md",true)  
end
...
if Controller(msg_chat_id,Message_Reply.sender.user_id) == 'المبرمج كريستينن' then
return LuaTele.sendText(msg_chat_id,msg_id,"\n* ❍︙ عذرا لا تستطيع حظر عام〘 "..Controller(msg_chat_id,Message_Reply.sender.user_id).." 〙*","md",true)  
end
...
if Controller(msg_chat_id,UserId) == 'المبرمج كريستينن' then
return LuaTele.sendText(msg_chat_id,msg_id,"\n* ❍︙ عذرا لا تستطيع حظر عام〘 "..Controller(msg_chat_id,UserId).." 〙*","md",true)  
end
...
local photo = LuaTele.getUserProfilePhotos(paris)
local mrt = LuaTele.getUser(paris)
for Name_User in string.gmatch(mrt.first_name, "[^%s]+" ) do
mrt.first_name = Name_User
break
end 
local NamesBot = (Redis:get(paris.."paris:Name:Bot") or "براديس")
local BotName = {
...
if text == (Redis:get(paris.."paris:Name:Bot") or "براديس") then
local photo = LuaTele.getUserProfilePhotos(paris)
local mrt = LuaTele.getUser(paris)
for Name_User in string.gmatch(mrt.first_name, "[^%s]+" ) do
mrt.first_name = Name_User
...
local Namebot = (Redis:get(paris.."Name:Bot") or "كلير")
local BotName = {
...
if text == (Redis:get(paris.."Name:Bot") or "كلير") then
local photo = LuaTele.getUserProfilePhotos(paris)
local ban = LuaTele.getUser(paris)
local Namebot = (Redis:get(paris.."Name:Bot") or "كلير")
local BotName = {
...
local CmdStart = '*\n🤖╖ أهلآ بك عزيزي أنا بوت'..(Redis:get(paris.."paris:Name:Bot") or "براديس")..
